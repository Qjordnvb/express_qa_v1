...
- **BasePage**: Foundation class with intelligent element finding using multiple selector fallbacks
...
- Base URL set to `https://www.google.com/` (change for your target site)
- Retry strategy: 0 retries locally, 2 retries in CI
- Custom timeout configurations and viewport settings
...
### Docker Requirements
- Only Docker Engine 20.10.0+ required for containerized execution
- No local Node.js installation needed for basic test execution
- Local Node.js + npm install required only for AI orchestration (`npm run orchestrate`)
...
- Runs as background service during orchestration
...
- Verify MCP server starts correctly (watch for MCP initialization logs)
