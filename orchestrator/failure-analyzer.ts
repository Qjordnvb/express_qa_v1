...
import { RealTimeContext } from './services/ContextService';
import { getLlmService } from './llm-service';
import { AIFailureAnalysis } from './llms/ILlmService';
import { MemoryRecord } from './services/MemoryService';
import { MCPElement } from './services/McpClientService';
...
class IntelligentMCPLearner {
  ...
  generateDynamicSelectors(discoveredElement: MCPElement): Selector[] {
    ...
    if (discoveredElement.name && discoveredElement.name !== 'Sin nombre') {
      selectors.push({
        type: 'getByRole',
    ...
    const css = cssMap[discoveredElement.role];
    if (css) {
      selectors.push({
        type: 'css',
        value: css
  ...
  private async learnFromDiscovery(elementName: string, discovery: DiscoveryResult): Promise<void> {
    ...
    if (existingPattern) {
      ...
      existingPattern.expectedRoles = [...new Set([...existingPattern.expectedRoles, discovery.element.role])];
  ...
  private calculateSemanticScore(element: MCPElement, functionName: string, knownPattern?: ElementPattern): number {
    ...
    if (expectedRoles.includes(element.role)) score += 0.4;
