...
test.describe('Category Page Visual Tests', () => {
  ...
  test('Monitors category page and first product card should be visually correct', async ({ page }: { page: Page }) => {
    ...
    const monitorsLink = page.getByRole('link', { name: 'Monitors', exact: true })
    ...
    await page.screenshot({ path: `debug-after-monitors-click-${browserNameForDebug}.png`, fullPage: true });

    // Verificamos el título principal de la página de Monitors
    const monitorsPageTitle = page.getByRole('heading', { level: 1, name: 'Monitors' })
    ...
    await expect(monitorsPageTitle).toHaveScreenshot(`monitors-page-title-${browserNameForDebug}.png`);
    ...
    await expect(firstProductCard).toHaveScreenshot(`monitors-first-product-card-${browserNameForDebug}.png`);
  });
