...
export class HybridOrchestrator {
  ...
  async generateTest(userStoryPath: string): Promise<GenerationResult> {
    ...
    try {
      ...
      // 2. Get stable context via Claude Code MCP with multi-page support
      console.log('🔍 Getting stable page context...');
      
      // Check if user story involves multiple pages/navigation
      const hasMultipleSteps = this.detectMultiPageFlow(userStory);
      
      let stableContext;
      let multiPageContext;
      
      if (hasMultipleSteps) {
        console.log('🌐 Detected multi-page flow, capturing all navigation states...');
        multiPageContext = await this.stableMcp.captureMultiPageFlow(userStory.fullUrl, userStory);
        
        // Use combined context from all pages
        stableContext = this.combineMultiPageContexts(multiPageContext);
      } else {
        console.log('📄 Single page flow detected, capturing single context...');
        stableContext = await this.stableMcp.getRealTimeContext(userStory.fullUrl);
      }
  ...
  private async generateAIAssets(userStory: any, stableContext: any): Promise<AIResponse> {
    ...
    const screenshotBase64 = stableContext.screenshot ? stableContext.screenshot.toString('base64') : '';
  ...
  /**
   * 🔍 Detect if user story involves multiple pages
   */
  private detectMultiPageFlow(userStory: any): boolean {
    const storyText = Array.isArray(userStory.userStory) 
      ? userStory.userStory.join(' ').toLowerCase()
      : userStory.userStory.toLowerCase();
    
    // Check for navigation indicators
    const multiPageIndicators = [
      'navego', 'navigate', 'página', 'page',
      'siguiente', 'next', 'continuar', 'continue',
      'redirige', 'redirect', 'acceder', 'access',
      'dashboard', 'home', 'inicio',
      'oauth', 'login', 'signin', 'authentication'
    ];
    
    // Check for multiple action steps
    const actionCount = (storyText.match(/clic|click|ingreso|type|fill|submit/gi) || []).length;
    
    return actionCount > 2 || multiPageIndicators.some(indicator => storyText.includes(indicator));
  }

  /**
   * 🔗 Combine multi-page contexts into single context for AI
   */
  private combineMultiPageContexts(multiPageContext: any): any {
    const { contexts, navigationEvents, flowSummary } = multiPageContext;
    
    // Combine all interactive elements from all pages
    const allElements: any[] = [];
    contexts.forEach((context: any, pageIndex: number) => {
      context.interactiveElements.forEach((element: any) => {
        allElements.push({
          ...element,
          pageIndex,
    ...
    // Create combined YAML snapshot
    const combinedYaml = contexts.map((ctx: any, idx: number) => 
      `# Page ${idx + 1}: ${ctx.pageName} (${ctx.url})\n${ctx.yamlSnapshot}`
    ).join('\n\n---\n\n');
    
    // Return enhanced context
    return {
      domSnapshot: combinedYaml,
      accessibilityTree: {},
      interactiveElements: allElements,
      eventLog: [],
      consoleErrors: contexts.flatMap((ctx: any) => ctx.consoleMessages.filter((msg: any) => msg.type === 'error')),
      networkErrors: contexts.flatMap((ctx: any) => ctx.networkRequests.filter((req: any) => req.status >= 400)),
      mcpConsoleMessages: contexts.flatMap((ctx: any) => ctx.consoleMessages),
      mcpNetworkRequests: contexts.flatMap((ctx: any) => ctx.networkRequests),
      mcpSnapshot: {
        yaml: combinedYaml,
        elements: allElements,
        url: contexts[contexts.length - 1]?.url || '',
      ...
      playwrightContext: {
        ...
        userAgent: 'Express QA Hybrid - Multi-page Flow'
      },
      multiPageFlow: {
        contexts,
        navigationEvents,
  ...
  /**
   * 🏗️ Build hybrid prompt combining Express QA AI + REAL MCP data
   */
  private buildHybridPrompt(userStory: any, stableContext: any): string {
    // 🎯 CONTEXTO REAL MCP: Usar elementos detectados con REAL MCP Protocol
    const realElements = stableContext.interactiveElements || [];
    const mcpSnapshot = stableContext.mcpSnapshot;
    
    // Formatear elementos detectados con información MCP real
    const elementsDescription = realElements.map((el: any, index: number) => {
      const refInfo = el.ref ? `[ref=${el.ref}]` : '';
      const selectorsInfo = el.selectors ? `- Selectors: ${JSON.stringify(el.selectors)}` : '';
      
      return `${index + 1}. ${el.type || 'element'} ${refInfo}
   - Text: "${el.text || 'Sin texto'}"
   - Role: "${el.role || 'N/A'}"
   - Name: "${el.name || 'N/A'}"
   ...
   ${selectorsInfo}`;
    ...
    // Check if this is a multi-page flow
    const isMultiPageFlow = !!stableContext.multiPageFlow;
    
    return `
**ANÁLISIS MCP REAL EN TIEMPO REAL (DATOS ESTRUCTURADOS PRIORITARIOS):**

**🎭 INFORMACIÓN MCP:**
- Modo: ${stableContext.playwrightContext?.userAgent?.includes('Real MCP') ? 'REAL MCP Protocol' : 'Fallback'}
- Tipo de flujo: ${isMultiPageFlow ? '🌐 MULTI-PÁGINA' : '📄 PÁGINA ÚNICA'}
- Snapshot YAML disponible: ${mcpSnapshot ? '✅' : '❌'}

${isMultiPageFlow ? `
**🌐 FLUJO MULTI-PÁGINA DETECTADO:**
- Total de páginas capturadas: ${stableContext.multiPageFlow.totalPages}
- Eventos de navegación: ${stableContext.multiPageFlow.navigationEvents.length}

**📊 PÁGINAS CAPTURADAS:**
${stableContext.multiPageFlow.contexts.map((ctx: any, idx: number) => 
  `${idx + 1}. ${ctx.pageName}: ${ctx.title} (${ctx.url}) - ${ctx.elementCount} elementos`
).join('\n')}

**🔄 NAVEGACIONES DETECTADAS:**
${stableContext.multiPageFlow.navigationEvents.map((event: any) => 
  `- ${event.action} en "${event.element}" → ${event.toPage}`
).join('\n') || 'Sin navegaciones detectadas'}
` : ''}

**🎯 ELEMENTOS INTERACTIVOS DETECTADOS CON REAL MCP (${realElements.length}):**
${elementsDescription}

**🏗️ INFORMACIÓN DE PÁGINA ${isMultiPageFlow ? 'FINAL' : 'ACTUAL'}:**
- URL: ${stableContext.pageInfo?.url || 'N/A'}
- Título: ${stableContext.pageInfo?.title || 'N/A'}
- Timestamp: ${stableContext.pageInfo?.timestamp || 'N/A'}
- Load Time: ${stableContext.pageInfo?.loadTime || 'N/A'}ms

**🎯 SNAPSHOT YAML MCP (DATOS ORIGINALES):**
${mcpSnapshot ? `\`\`\`yaml\n${mcpSnapshot.yaml.substring(0, 1000)}${mcpSnapshot.yaml.length > 1000 ? '\n... (truncated)' : ''}\n\`\`\`` : 'No disponible'}

**🚀 INSTRUCCIONES ESPECIALES PARA USAR DATOS REAL MCP:**
1. **PRIORIDAD ABSOLUTA:** Los elementos con [ref=xxx] son de REAL MCP y deben usarse para interacciones
2. **REFERENCIAS MCP:** Cada elemento tiene un 'ref' que debe incluirse en los page objects para interacciones MCP
3. **ROLES ARIA EXACTOS:** Usa los roles proporcionados (button, textbox, link, etc.) en getByRole
4. **NOMBRES PRECISOS:** Si un elemento tiene 'text', úsalo en getByText o getByRole con options: { name: "texto_exacto" }
5. **SELECTORES RESILIENTES:** Genera múltiples selectores basados en los datos MCP reales
6. **SELECTORES VÁLIDOS PLAYWRIGHT:** Usa :has-text() en lugar de :contains() para selectores CSS
7. **INCLUIR REFS:** SIEMPRE incluye el campo 'ref' de MCP en los locators generados
...
Tienes acceso COMPLETO al protocolo MCP (Model Context Protocol) real que te permite:

1. **NAVEGACIÓN MCP REAL:**
   - Navegar a cualquier URL usando herramientas MCP nativas
   - Obtener snapshots YAML reales con referencias exactas [ref=eXX]
   - Capturar screenshots reales del navegador

2. **INTERACCIÓN MCP REAL:**
   - Hacer clic en elementos usando referencias MCP exactas
   - Escribir texto en campos usando el protocolo MCP
   - Detectar elementos interactivos con datos estructurados

3. **ANÁLISIS MCP AVANZADO:**
   - Obtener accessibility trees reales
   - Capturar network requests y console messages
   - Análisis de DOM en tiempo real

4. **GENERACIÓN INTELIGENTE:**
   - Los elementos con [ref=eXX] son referencias MCP REALES para interacciones
   - Puedes generar selectores múltiples basados en datos MCP reales
   - Tienes acceso a YAML snapshots estructurados del navegador real
...
${isMultiPageFlow ? `
**🌐 FLUJO MULTI-PÁGINA - INSTRUCCIONES CRÍTICAS:**
1. Se han capturado ${stableContext.multiPageFlow.totalPages} páginas diferentes durante el flujo
2. DEBES generar un pageObject para CADA página capturada
3. Los elementos están etiquetados con su página de origen (pageIndex, pageName)
4. Usa la propiedad "page" en testSteps para indicar en qué página se ejecuta cada paso
5. Los elementos de diferentes páginas tienen diferentes refs - úsalos correctamente

**PÁGINAS A GENERAR:**
${stableContext.multiPageFlow.contexts.map((ctx: any, idx: number) => 
  `- ${ctx.pageName} (${ctx.url}): Generar clase ${ctx.pageName.replace(/[^a-zA-Z0-9]/g, '')}Page`
).join('\n')}
` : ''}

**EJEMPLO DE LÓGICA REQUERIDA:**
- HU: "ingreso email → ingreso password → accedo al dashboard"  
- MCP: ${isMultiPageFlow ? 'detectó navegación real entre páginas' : 'detecta solo botón "Ingresa por Ab-Inbev"'}
- **TU DEBES GENERAR:** ${isMultiPageFlow ? 
  stableContext.multiPageFlow.contexts.map((ctx: any) => ctx.pageName).join(' + ') : 
  'SmartCommsLoginPage + MicrosoftLoginPage (con email/password) + DashboardPage (con verificación)'}
