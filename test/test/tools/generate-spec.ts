...
const specSteps = testSteps.map((step, index) => {
 ...
 return stepCode;
  ...
  `;
// Crear carpeta específica para este test basada en el nombre del testcase
const testFolderName = path.basename(testCasePath, '.testcase.json');
const outputDir = path.resolve(__dirname, `../tests/generated/${testFolderName}`);
